...
def obj_edge_vectors(names, wv_type='glove.6B', wv_dir=None, wv_dim=300):
    ...
    for i, token in enumerate(names):
        ...
        if token=='unsure':
            vectors[i]=torch.mean(vectors[:2],dim=0,keepdim=True)
        elif token=='otherrelationship':
            waiti=i
        elif wv_index is not None:
    ...
    if waiti>0:
        vectors[waiti]=torch.mean(vectors[9:],dim=0,keepdim=True)
