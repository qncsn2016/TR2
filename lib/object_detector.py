...
class detector(nn.Module):
    ...
    def __init__(self, train, object_classes, use_SUPPLY, mode='predcls'):
        ...
        self.fasterRCNN = resnet(
            classes=self.object_classes, num_layers=101, pretrained=False, class_agnostic=False)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, gt_annotation, deb=None):
        ...
        if self.mode == 'sgdet':
                ...
                for i in range(rois.shape[0]):
                    ...
                    for j in range(1, len(self.object_classes)):
                        ...
                        if inds.numel() > 0:
                            ...
                            final_bbox = torch.cat((torch.tensor(
                                [[counter_image]], dtype=torch.float).repeat(final_bbox.shape[0], 1).cuda(0),
                                final_bbox), 1)
            ...
            if self.is_train:
                ...
                DETECTOR_FOUND_IDX, GT_RELATIONS, SUPPLY_RELATIONS, assigned_labels = \
                    assign_relations(prediction, gt_annotation, assign_IOU_threshold=0.5)
                    ...
                    for i, j in enumerate(SUPPLY_RELATIONS):
                        if len(j) > 0:
                            ...
                            for m, n in enumerate(j):
                                ...
                                if 'bbox' in n.keys():  # don't forget scaling!
                                    unfound_gt_bboxes[m, 1:] = torch.tensor(n['bbox']).cuda(0) * im_info[i, 2]
                            ...
                            DETECTOR_FOUND_IDX[i] = list(np.concatenate((
                                DETECTOR_FOUND_IDX[i],
                                np.arange(start=int(sum(FINAL_BBOXES[:, 0] == i)),
                                    stop=int(sum(FINAL_BBOXES[:, 0] == i)) + len(SUPPLY_RELATIONS[i]))
                                    ), axis=0).astype('int64'))
            ...
            FINAL_FEATURES_2d = self.fasterRCNN.RCNN_roi_align(FINAL_BASE_FEATURES, FINAL_BBOXES)
            FINAL_FEATURES_1d = self.fasterRCNN._head_to_tail(FINAL_FEATURES_2d)

            whole_box=torch.cat([
                torch.unsqueeze(torch.arange(FINAL_BASE_FEATURES.shape[0]),1),
                torch.tile(torch.tensor([0,0,im_info[0,1],im_info[0,0]]),(FINAL_BASE_FEATURES.shape[0],1))
            ],dim=1).to(FINAL_BASE_FEATURES.device)
            BACK_FEATURES=self.fasterRCNN.RCNN_roi_align(FINAL_BASE_FEATURES, whole_box)
            BACK_FEATURES=self.fasterRCNN._head_to_tail(BACK_FEATURES)
            ...
            if self.mode == 'predcls':
                ...
                spatial_masks = torch.tensor(
                    draw_union_boxes(pair_rois, 27) - 0.5).to(FINAL_FEATURES_2d.device)
                ...
                entry = {'boxes': FINAL_BBOXES,
                         ...
                         'pair_idx': pair,
                         'human_idx': HUMAN_IDX,
                         'features_2d': FINAL_FEATURES_2d,
                         'features_1d': FINAL_FEATURES_1d,
                         'union_feat': union_feat,
                         'union_box': union_boxes,
                         'spatial_masks': spatial_masks,
                         'attention_gt': a_rel,
                         'spatial_gt': s_rel,
                         'contacting_gt': c_rel,
                         'base_features':FINAL_BASE_FEATURES,
                        }
                ...
                if self.is_train:
                    ...
                    FINAL_DISTRIBUTIONS = torch.softmax(self.fasterRCNN.RCNN_cls_score(FINAL_FEATURES_1d)[:, 1:], dim=1)
                    FINAL_SCORES, PRED_LABELS = torch.max(FINAL_DISTRIBUTIONS, dim=1)
                    PRED_LABELS = PRED_LABELS + 1
                    ...
                    pair_rois = torch.cat((FINAL_BBOXES[pair[:, 0], 1:], FINAL_BBOXES[pair[:, 1], 1:]),
                                          1).data.cpu().numpy()
                    spatial_masks = torch.tensor(draw_union_boxes(pair_rois, 27) - 0.5).to(FINAL_FEATURES_1d.device)
