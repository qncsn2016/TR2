...
class BasicSceneGraphEvaluator:
    def __init__(self, mode, AG_object_classes, AG_all_predicates, AG_attention_predicates, AG_spatial_predicates, AG_contacting_predicates,
        ...
        self.result_dict = {}
        self.mode = mode
        self.result_dict[self.mode + '_recall'] = {10: [], 20: [], 50: []}
...
def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, method=None, threshold = 0.9, **kwargs):
    ...
    if method == 'no':
        ...
        if len(pred_rel_inds)==0:
            pred_rels=np.zeros((0,3))
            predicate_scores=np.zeros((0))
        else:
            obj_scores_per_rel = obj_scores[pred_rel_inds].prod(1)
            overall_scores = obj_scores_per_rel[:, None] * rel_scores
            num_rel = pred_rel_inds.shape[0]//3
            a1=[rel_scores[i::num_rel] for i in range(num_rel)]
            a2=[np.sum(a1[i],0)[None,:] for i in range(len(a1))]
            a3=np.concatenate(a2) # num_rel, 26
            h_idx=pred_rel_inds[0][0]
            o_idx=pred_rel_inds[:num_rel,1]
            top_idx = np.argsort(-a3, 1)[:,:6]
            top_scores = -np.sort(-a3, 1)[:,:6]
            a4=np.tile(o_idx[:,None],(1,6))
            a5=np.reshape(np.concatenate((a4[...,None],top_idx[...,None]),-1),(-1,2))
            a6=np.tile(np.array([[h_idx]]), (num_rel*6,1))
            pred_rels = np.concatenate((a6,a5),1)
            for pi in range(len(pred_rels)):
                if pred_rels[pi][-1]>2 and pred_rels[pi][-1]<9:
                    pred_rels[pi][:2] = pred_rels[pi][:2][::-1]
            predicate_scores = top_scores.flatten()
    else:
